...
import com.acme.blogging.Initialization.domain.repository.CollaboratorRepository;
import com.acme.blogging.Initialization.domain.service.CollaboratorService;
...
public class CollaboratorServiceImpl implements CollaboratorService {
    ...
    public Collaborator createCollaborator(Collaborator collaborator) {
        ...
        Collaborator existingUsername = collaboratorRepository.findCollaboratorByUsername(collaborator.getUsername()).orElse(null);
        if(existingUsername != null){
            throw new ResourceNotFoundException("The username " + existingUsername.getUsername() + " is begin used by another user");
        }
    ...
    public Collaborator getCollaboratorByUsernameAndPassword(String username, String password) {
        return collaboratorRepository.findCollaboratorByUsernameAndEmail(username,password)
                .orElseThrow(()->new ResourceNotFoundException("Customer","Username or Pasword", username + " " + password));
    }

    @Override
    public ResponseEntity<?> deleteCollaborator(String username) {
        Collaborator _collaborator = collaboratorRepository.findCollaboratorByUsername(username).orElseThrow(()->new ResourceNotFoundException("Collaborator","Username",username));
